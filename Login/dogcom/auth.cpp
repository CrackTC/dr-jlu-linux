...
void print_packet(const char msg[10], const unsigned char *packet, int length) {
	...
	for (int i = 0; i < length; i++) {
        std::sprintf(str + 2 * i, "%02x", packet[i]);
	}
	*(str + 2 * length) = '\0';
    std::printf("%s\n", buf);
}
...
int dhcp_challenge(int sockfd, struct sockaddr_in addr, unsigned char seed[]) {
	...
	if (recv_packet[0] != 0x02) {
        std::printf("Bad challenge response received.\n");
		return 1;
	}
	else {
		// 收到的报文中有ip地址
        ...
        std::printf("IP: %d.%d.%d.%d", receivedIp[0], receivedIp[1], receivedIp[2], receivedIp[3]);
	}
...
int dogcom() {
	...
	std::printf("You are binding at %s!\n\n", drcom_config.host_ip);
	bind_addr.sin_addr.s_addr = inet_addr(drcom_config.host_ip);
